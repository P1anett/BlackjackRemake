...
void LevelScene::startRound()
{
    ...
    if (playerHands[currentHand].getValue() == 21 && dealerHand.getValue() != 21) {
        wallet->setTotal(wallet->getTotal() + betTotal*2.5);
...
{
    if (!playerHands.empty() && playerHands[currentHand].getCards().size() == 2) {
...
{
    if (!playerHands.empty() && playerHands[currentHand].canSplit()) {
        Hand newHand;

        std::vector<Card*> cards = playerHands[currentHand].getCards();
        if (cards.size() == 2) {
            newHand.addCard(cards[1]);
            playerHands[currentHand] = Hand();
            playerHands[currentHand].addCard(cards[0]);
...
void LevelScene::dealDealer()
{
    ...
    if (!dealerHand.getCards().empty() && dealerHand.getCards().size() > 1) { 
        dealerHand.getCards()[1]->setFrame(dealerHand.getCards()[1]->getObjectID());
    }

    while (dealerHand.getValue() < 17) {
        Card* card = deck->dealCard();
        card->setPosition(10 + (dealerHand.getCards().size() * 150), 80.0f);
        card->setFrame(card->getObjectID());
        dealerHand.addCard(card);
        addGameObject(card, 3);
    }

    LOG_INFO("Dealer stands at " + std::to_string(dealerHand.getValue()));

    if (playerHands[currentHand].getValue() > 21) {
        LOG_INFO("PLAYER LOSES");
        displayButtons(true, false, false, false, false, false);
    }
    else if (dealerHand.getValue() > 21 && playerHands[currentHand].getValue() <= 21) {
        LOG_INFO("PLAYER WINS");
        wallet->setTotal(wallet->getTotal() + betTotal * 2);
        displayButtons(true, false, false, false, false, false);
    }
    else if (dealerHand.getValue() == playerHands[currentHand].getValue()) {
        LOG_INFO("Its a push");
        wallet->setTotal(wallet->getTotal() + betTotal);
        displayButtons(true, false, false, false, false, false);
    }
    else if (playerHands[currentHand].getValue() > dealerHand.getValue()) {
        LOG_INFO("PLAYER WINS");
        wallet->setTotal(wallet->getTotal() + betTotal * 2);
        displayButtons(true, false, false, false, false, false);
    }
